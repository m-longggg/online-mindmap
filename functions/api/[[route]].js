...
async function handleMindMaps(request, env, user) {
  ...
  switch (method) {
    case 'GET':
      return await getMindMaps(request, env, user);
    case 'POST':
...
// 获取用户的所有思维导图
async function getMindMaps(request, env, user) {
  try {
    const url = new URL(request.url);
    ...
    if (id) {
      ...
      const mindmap = await env.DB.prepare(
        'SELECT id, title, content, created_at, updated_at, is_public FROM mindmaps WHERE id = ? AND user_id = ?'
    ...
    return new Response(JSON.stringify({ error: 'Failed to get mind maps' }), { 
      status: 500,
      headers: { 'Content-Type': 'application/json' }
    });
  }
...
async function createMindMap(request, env, user) {
  try {
    ...
    return new Response(JSON.stringify({ error: 'Failed to create mind map' }), { 
      status: 500,
      headers: { 'Content-Type': 'application/json' }
    });
  }
...
async function updateMindMap(request, env, user) {
  try {
    ...
    if (!id) {
      return new Response(JSON.stringify({ error: 'Mind map ID is required' }), { 
        status: 400,
        headers: { 'Content-Type': 'application/json' }
      });
    }
    ...
    const result = await env.DB.prepare(
      'UPDATE mindmaps SET title = ?, content = ?, is_public = ?, updated_at = datetime("now") WHERE id = ? AND user_id = ?'
    ).bind(title, content, is_public ? 1 : 0, id, user.id).run();
    ...
    return new Response(JSON.stringify({ error: 'Failed to update mind map' }), { 
      status: 500,
      headers: { 'Content-Type': 'application/json' }
    });
  }
...
async function deleteMindMap(request, env, user) {
  try {
    ...
    return new Response(JSON.stringify({ error: 'Failed to delete mind map' }), { 
      status: 500,
      headers: { 'Content-Type': 'application/json' }
    });
  }
...
async function hashPassword(password) {
  ...
  // 这里使用简单的SHA-256作为示例，生产环境请使用更安全的方法
